//! ram 的通用工具函数。
//!
//! 这里放不属于具体业务模块的小函数，例如 URL 编解码、glob 匹配、TLS 文件加载、
//! HTTP Range 头解析等。

use anyhow::{anyhow, Context, Result};

#[cfg(feature = "tls")]
use rustls_pki_types::{pem::PemObject, CertificateDer, PrivateKeyDer};
use std::{
    borrow::Cow,
    path::Path,
    time::{Duration, SystemTime, UNIX_EPOCH},
};

/// 返回当前 Unix 时间间隔。
pub fn unix_now() -> Duration {
    // 返回从 Unix 纪元到现在的时间间隔，认证 nonce 和 token 都会用到它。
    SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("Unable to get unix epoch time")
}

/// 对 URL 路径做百分号编码，同时保留 `/` 分隔符。
pub fn encode_uri(v: &str) -> String {
    // 按路径段分别编码，保留 / 分隔符，否则整个路径会变成一个段。
    let parts: Vec<_> = v.split('/').map(urlencoding::encode).collect();
    parts.join("/")
}

/// 对 URL 百分号编码内容解码。
pub fn decode_uri(v: &str) -> Option<Cow<'_, str>> {
    // URL 百分号解码；如果不是合法 UTF-8，就返回 None。
    percent_encoding::percent_decode(v.as_bytes())
        .decode_utf8()
        .ok()
}

/// 获取文件名；取不到时返回空字符串。
pub fn get_file_name(path: &Path) -> &str {
    path.file_name()
        .and_then(|v| v.to_str())
        .unwrap_or_default()
}

/// 获取文件名；取不到时返回错误。
pub fn try_get_file_name(path: &Path) -> Result<&str> {
    path.file_name()
        .and_then(|v| v.to_str())
        .ok_or_else(|| anyhow!("Failed to get file name of `{}`", path.display()))
}

/// 用 glob 模式匹配字符串。
pub fn glob(pattern: &str, target: &str) -> bool {
    // hidden 规则使用 glob 匹配，例如 *.log 可以匹配 app.log。
    let pat = match ::glob::Pattern::new(pattern) {
        Ok(pat) => pat,
        Err(_) => return false,
    };
    pat.matches(target)
}

// 从文件加载 TLS 公共证书。
#[cfg(feature = "tls")]
pub fn load_certs<T: AsRef<Path>>(file_name: T) -> Result<Vec<CertificateDer<'static>>> {
    let mut certs = vec![];
    for cert in CertificateDer::pem_file_iter(file_name.as_ref()).with_context(|| {
        format!(
            "Failed to load cert file at `{}`",
            file_name.as_ref().display()
        )
    })? {
        let cert = cert.with_context(|| {
            format!(
                "Invalid certificate data in file `{}`",
                file_name.as_ref().display()
            )
        })?;
        certs.push(cert)
    }
    if certs.is_empty() {
        anyhow::bail!(
            "No supported certificate in file `{}`",
            file_name.as_ref().display()
        );
    }
    Ok(certs)
}

// 从文件加载 TLS 私钥。
#[cfg(feature = "tls")]
pub fn load_private_key<T: AsRef<Path>>(file_name: T) -> Result<PrivateKeyDer<'static>> {
    PrivateKeyDer::from_pem_file(file_name.as_ref()).with_context(|| {
        format!(
            "Failed to load key file at `{}`",
            file_name.as_ref().display()
        )
    })
}

/// 解析 HTTP Range 头。
///
/// 返回的元组是闭区间 `(start, end)`，表示要读取文件的哪一段。
pub fn parse_range(range: &str, size: u64) -> Option<Vec<(u64, u64)>> {
    // 解析 HTTP Range 头，例如：
    // bytes=0-99     表示前 100 字节；
    // bytes=100-     表示从第 100 字节到末尾；
    // bytes=-50      表示最后 50 字节。
    let (unit, ranges) = range.split_once('=')?;
    if unit != "bytes" {
        return None;
    }
    if size == 0 {
        return None;
    }

    let mut result = Vec::new();
    for range in ranges.split(',') {
        let (start, end) = range.trim().split_once('-')?;
        if start.is_empty() {
            let offset = end.parse::<u64>().ok()?;
            if offset > 0 && offset <= size {
                result.push((size - offset, size - 1));
            } else {
                return None;
            }
        } else {
            let start = start.parse::<u64>().ok()?;
            if start < size {
                if end.is_empty() {
                    result.push((start, size - 1));
                } else {
                    let end = end.parse::<u64>().ok()?;
                    if end < size && start <= end {
                        result.push((start, end));
                    } else {
                        return None;
                    }
                }
            } else {
                return None;
            }
        }
    }

    Some(result)
}

/// 检查当前系统是否能绑定 IPv6 本地地址。
pub fn is_ipv6_available() -> bool {
    // 尝试创建 IPv6 socket，成功就认为当前系统支持 IPv6 监听。
    use socket2::{Domain, Protocol, Socket, Type};
    Socket::new(Domain::IPV6, Type::STREAM, Some(Protocol::TCP)).is_ok()
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn glob_matches_supported_patterns() {
        assert!(glob("", ""));
        assert!(glob(".*", ".git"));
        assert!(glob("abc", "abc"));
        assert!(glob("a*c", "abc"));
        assert!(glob("a?c", "abc"));
        assert!(glob("a*c", "abbc"));
        assert!(glob("*c", "abc"));
        assert!(glob("a*", "abc"));
        assert!(glob("?c", "bc"));
        assert!(glob("a?", "ab"));
        assert!(!glob("abc", "adc"));
        assert!(!glob("abc", "abcd"));
        assert!(!glob("a?c", "abbc"));
        assert!(!glob("*.log", "log"));
        assert!(glob("*.abc-cba", "xyz.abc-cba"));
        assert!(glob("*.abc-cba", "123.xyz.abc-cba"));
        assert!(glob("*.log", ".log"));
        assert!(glob("*.log", "a.log"));
        assert!(glob("*/", "abc/"));
        assert!(!glob("*/", "abc"));
    }

    #[test]
    fn parse_range_supports_single_and_multi_ranges() {
        assert_eq!(parse_range("bytes=0-499", 500), Some(vec![(0, 499)]));
        assert_eq!(parse_range("bytes=0-", 500), Some(vec![(0, 499)]));
        assert_eq!(parse_range("bytes=299-", 500), Some(vec![(299, 499)]));
        assert_eq!(parse_range("bytes=-500", 500), Some(vec![(0, 499)]));
        assert_eq!(parse_range("bytes=-300", 500), Some(vec![(200, 499)]));
        assert_eq!(
            parse_range("bytes=0-199, 100-399, 400-, -200", 500),
            Some(vec![(0, 199), (100, 399), (400, 499), (300, 499)])
        );
    }

    #[test]
    fn parse_range_rejects_out_of_bounds_or_malformed_ranges() {
        assert_eq!(parse_range("bytes=500-", 500), None);
        assert_eq!(parse_range("bytes=-501", 500), None);
        assert_eq!(parse_range("bytes=0-500", 500), None);
        assert_eq!(parse_range("bytes=0-199,", 500), None);
        assert_eq!(parse_range("bytes=0-199, 500-", 500), None);
        assert_eq!(parse_range("items=0-1", 500), None);
        assert_eq!(parse_range("bytes=-0", 500), None);
        assert_eq!(parse_range("bytes=0-", 0), None);
        assert_eq!(parse_range("bytes=-1", 0), None);
    }

    #[test]
    fn uri_encoding_round_trips_unicode_path_segments() {
        let encoded = encode_uri("目录/a b.txt");

        assert_eq!(encoded, "%E7%9B%AE%E5%BD%95/a%20b.txt");
        assert_eq!(decode_uri(&encoded).as_deref(), Some("目录/a b.txt"));
    }
}
