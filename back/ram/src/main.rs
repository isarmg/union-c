//! ram 可执行程序入口。
//!
//! 这个文件负责把命令行参数、监听地址、HTTP/TLS 连接和 `Server` 请求处理器串起来。
//! 真正的文件浏览、上传、下载等业务逻辑在 `server.rs`；这里更像“启动器”。

#[cfg(not(target_os = "linux"))]
compile_error!("ram supports Linux only");
...
async fn main() -> Result<()> {
    ...
    tokio::select! {
        ...
        // 正常退出路径：用户按 Ctrl+C，running 置为 false，让长时间任务感知退出。
        _ = shutdown_signal() => {
...
fn serve(args: Args, running: Arc<AtomicBool>) -> Result<Vec<JoinHandle<()>>> {
    // 一个 ram 进程可以同时监听多个地址，例如 IPv4 + IPv6。
    ...
    for bind_addr in addrs.iter() {
        ...
        match bind_addr {
            BindAddr::IpAddr(ip) => {
                // TCP 监听地址，例如 0.0.0.0:5000 或 [::]:5000。
                ...
                match &tls_config {
                    ...
                    (Some(cert_file), Some(key_file)) => {
                        // 如果启用了 TLS，每个连接先做 TLS 握手，再交给 HTTP 处理。
                    ...
                    (None, None) => {
                        let handle = tokio::spawn(async move {
                            loop {
                                // accept 等待新客户端连接；每个连接再 spawn 一个任务处理。
                                let Ok((stream, addr)) = listener.accept().await else {
                                    continue;
                                };
                                let stream = TokioIo::new(stream);
            ...
            BindAddr::SocketPath(path) => {
                // Linux Unix socket 适合被本机反向代理调用；@ 前缀表示抽象命名空间。
...
{
    // hyper 的 service_fn 会把每个 HTTP 请求转发到 Server::call。
    let hyper_service =
        ...
        .await
    {
        Ok(()) => {}
        Err(_err) => {
            // 客户端连上后没有发送任何请求就断开时，hyper 会返回这里的错误。
            // 这类错误对服务端没有实际影响，所以这里静默忽略。
        }
    }
}

fn create_listener(addr: SocketAddr) -> Result<TcpListener> {
    // 使用 socket2 是为了显式设置 SO_REUSEADDR、IPv6 only 等底层 socket 选项。
...
fn check_addrs(args: &Args) -> Result<(Vec<BindAddr>, Vec<BindAddr>)> {
    // new_addrs 是实际要绑定的地址，print_addrs 是显示给用户看的可访问地址。
    ...
    let (ipv4_addrs, ipv6_addrs) = if has_unspecified {
        // 0.0.0.0 或 :: 表示监听所有网卡；打印时展开成本机真实网卡地址更直观。
        interface_addrs()?
    } else {
        (vec![], vec![])
    };
...
fn interface_addrs() -> Result<(Vec<BindAddr>, Vec<BindAddr>)> {
    // 枚举本机所有网卡地址，用于把“监听全部地址”转换成用户可打开的 URL。
...
fn print_listening(args: &Args, print_addrs: &[BindAddr]) -> Result<String> {
    // 根据是否启用 TLS、是否 IPv6，拼出启动后打印在终端的访问地址。
