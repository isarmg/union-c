    ...
    <section className="view-stack">
      <section className="section-band metric-section">
        ...
        <div className="content-grid metric-grid">
          <Metric
            label="服务"
            value={`${healthyCount}/${services.length || 3}`}
          ...
          <Metric
            ...
            value={
              resources
                ? formatBytesPerSecond(resources.disk_throughput?.total_bytes_per_second ?? 0)
                : "--"
            }
            detail={
              resources
                ? `读 ${formatBytesPerSecond(resources.disk_throughput?.read_bytes_per_second ?? 0)}  写 ${formatBytesPerSecond(resources.disk_throughput?.write_bytes_per_second ?? 0)}`
                : undefined
            }
            tone="neutral"
