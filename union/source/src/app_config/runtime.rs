...
pub fn ensure_layout(settings: &Settings) -> std::io::Result<()> {
    if settings.production && require_local_static_artifacts() {
        ensure_production_artifacts()?;
    }

    for dir in data_roots() {
...
fn ensure_production_artifacts() -> std::io::Result<()> {
    for required in [
        Path::new("blog/source/package.json"),
        Path::new("blog/source/dist/index.html"),
        Path::new("back/source/dist/index.html"),
    ] {
        if !required.is_file() {
...
fn require_local_static_artifacts() -> bool {
    env::var("UNION_REQUIRE_LOCAL_STATIC_ARTIFACTS")
        .is_ok_and(|value| matches!(value.as_str(), "1" | "true" | "TRUE" | "yes" | "YES"))
}

fn data_roots() -> [&'static Path; 4] {
    [
        Path::new("back/data"),
        Path::new("blog/data"),
        Path::new("ram/data"),
...
fn runtime_directories(settings: &Settings) -> [&Path; 10] {
    [
