...
本页描述单机部署。四个子项目分别部署到四台 Linux 时，见 [四机独立部署](four-linux-deployment.md)，并保持 `UNION_REQUIRE_LOCAL_STATIC_ARTIFACTS=0`。
...
```bash
cargo build --release --locked --manifest-path union/source/Cargo.toml
cargo build --release --locked --manifest-path ram/source/Cargo.toml

(cd back/source && npm ci && npm run build)
(cd blog/source && npm ci && npm run build)
```

前端发布脚本会把 `dist` 树规范化为目录 `0755`、文件 `0644`。不要依赖构建时
`umask` 让 Caddy 读取静态文件；生产脚本会显式设置可读权限。
...
sudo install -o root -g root -m 0755 \
  /opt/union/union/source/target/release/union \
  /opt/union/bin/union
sudo install -o root -g root -m 0755 \
  /opt/union/ram/source/target/release/ram \
  /opt/union/bin/ram
```
...
单机部署可设置 `UNION_REQUIRE_LOCAL_STATIC_ARTIFACTS=1`，让 Union 启动时检查本机管理端和博客静态产物；四机部署保持默认 `0`。
...
博客运行时发布需要在 `blog/source` 下原子切换 `dist`、`dist.next` 和
`dist.previous`；部署脚本会把该目录设为 root 拥有、服务组可写并带 sticky bit，
源码文件仍由 root 拥有，运行账号只拥有 `dist`、`.astro` 和 `node_modules/.vite`
等必要写入位置。
...
- 博客主机托管 `blog/source/dist`；
- 管理主机托管 `back/source/dist`，并把 `/api/*` 代理到 `127.0.0.1:8080`；
