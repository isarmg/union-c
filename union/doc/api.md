...
以管理端读取文章为例：页面调用 `back/source/src/api.ts` 中的 API 函数，请求到达 `union/source/src/routes/mod.rs` 后，先依次经过请求 ID、日志、请求体大小和访问控制中间件，再进入 `routes/blog.rs` 的具体处理函数。处理函数调用博客业务模块和 `database/blog/`，最后把 Rust 类型序列化为 JSON。
