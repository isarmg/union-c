...
后续所有 Rust 命令都应从仓库根目录执行。应用的相对路径以当前工作目录解析，从其他目录启动会把各项目 `data/` 写到错误位置。

开发模式未提供 `UNION_SECRET_KEY` 时，`union` 会创建权限为 `0600` 的 `union/data/union.secret`。此文件只能用于本机开发，不能提交或复制到不受信任的位置。
...
```bash
cargo fetch --manifest-path union/source/Cargo.toml --locked
cargo fetch --manifest-path ram/source/Cargo.toml --locked

(cd back/source && npm ci)
(cd blog/source && npm ci)
```
...
export PATH="$PWD/ram/source/target/debug:$PATH"
```

从仓库根目录启动 `union`，确保相对路径都落在四个项目自己的 `data/` 和 `source/`：

```bash
cargo run --manifest-path union/source/Cargo.toml --locked
```

首次启动会把管理员 bcrypt 哈希写入权限为 `0600` 的 `union/data/union-config.json`。开发模式若未设置初始密码，会在终端打印一次随机密码。从控制台配置数据库时会执行基线建表、写入默认运行配置并立即切换连接，无需重启。
...
Vite 会把管理端 `/api` 请求代理到 `union`。博客开发服务器直接读取 `blog/data/content/` 与 `blog/data/files/`。

## 测试与静态检查

```bash
cargo fmt --manifest-path union/source/Cargo.toml -- --check
cargo test --manifest-path union/source/Cargo.toml --all-targets --locked
cargo clippy --manifest-path union/source/Cargo.toml --all-targets --all-features --locked -- -D warnings
...
(cd back/source && npm run build)
(cd blog/source && npm run build)
```
...
UNION_TEST_DATABASE_URL='postgresql://union:password@127.0.0.1:5432/union_test' \
  cargo test --manifest-path union/source/Cargo.toml --test database_migrations --locked -- --nocapture
```
...
```bash
cargo audit --file union/source/Cargo.lock
cargo audit --file ram/source/Cargo.lock
(cd back/source && npm audit --audit-level=high)
(cd blog/source && npm audit --audit-level=high)
```
...
- 修改 API 响应结构时，同步检查 `union/source/src/domain/` 对应业务文件与 `back/source/src/types.ts`。
