...
├── union/
│   ├── source/       Rust 管理后端
│   ├── data/         本地私有配置、密钥和控制面运行数据
│   └── doc/          控制面、部署和跨项目文档
├── ram/
│   ├── source/       Rust 文件服务（ram 内核）
│   ├── data/         文件、日志和生成配置
│   └── doc/          文件服务文档
├── blog/
│   ├── source/       Astro 静态博客
...
| 目录 | 技术 | 独立入口 | 产物 |
| --- | --- | --- | --- |
| `back/source` | React、TypeScript、Vite | `package.json`、`package-lock.json` | `dist/` |
| `union/source` | Rust、Axum、SQLx | `Cargo.toml`、`Cargo.lock` | `target/release/union` |
| `ram/source` | Rust、Hyper | `Cargo.toml`、`Cargo.lock` | `target/release/ram` |
| `blog/source` | Astro、TypeScript | `package.json`、`package-lock.json` | `dist/` |
...
只读的静态站点构建器。文章来自 `blog/data/content/`，图片和附件来自 `blog/data/files/`。生产环境由 Caddy 直接托管构建后的 `dist/`。
...
union ──导出文章──> blog/data/content ──> blog 构建
union ──生成配置/启停──> ram ──读写──> ram/data/files
...
| 状态 | 唯一管理源 | 派生物 |
| --- | --- | --- |
| Union 管理员凭据 | `union/data/union-config.json`（0600） | 内存 bcrypt 校验数据 |
| Union 登录会话 | 进程内存 | 重启后全部失效 |
| 运行配置 | PostgreSQL 加密设置 | 内存配置、ram 私有 YAML |
| Sunshine/Proxmox 主机 | PostgreSQL `external_hosts` | 内存主机快照 |
| 博客文章和分类标签 | PostgreSQL | `blog/data/content/*` |
| 博客静态资源 | `blog/data/files/` | `blog/source/dist/blog-assets/` |
| 文件服务内容 | `ram/data/files/` | 无 |
| 管理台和博客页面 | 源代码 | 各自 `dist/` |

数据库与各项目 `data/` 中的运行数据必须一起备份。派生内容可以重建，但不能用派生内容替代数据库备份。
...
博客发布不是直接修改线上 `dist/`：PostgreSQL 内容先导出到 `blog/data/content/`，Astro 构建到 `dist.next`，验证成功后再切换为 `dist`。
