...
pub fn disconnected_pool() -> anyhow::Result<DbPool> {
    Ok(PgPoolOptions::new()
        ...
        .connect_lazy("postgresql://unionc:unused@127.0.0.1:1/unionc_unconfigured")?)
}
...
// 目录不会被运行时自动扫描。include_str! 在编译期把 SQL 放进 unionc 二进制，部署时
// 不需要复制 migrations 目录；新增 SQL 文件后也必须在此数组登记版本和说明。
const MIGRATIONS: &[Migration] = &[
    Migration {
        version: 1,
        description: "unionc baseline schema",
        sql: include_str!("../../migrations/0001_baseline.sql"),
    },
    Migration {
        version: 2,
        description: "read-only host monitoring",
        sql: include_str!("../../migrations/0002_host_monitoring.sql"),
    },
];
