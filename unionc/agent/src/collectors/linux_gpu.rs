...
fn collect_card(card: &str, device: &Path, vendor: &str) -> GpuSnapshot {
    ...
    let temperature_celsius = first_hwmon_value(device, "temp1_input")
        .map(|value| value as f64 / 1_000.0)
        .filter(|value| value.is_finite());
    let core_clock_mhz = read_u64(device.join("gt_cur_freq_mhz"));
    GpuSnapshot {
        id: pci_slot(device).unwrap_or_else(|| card.to_string()),
        vendor: vendor.to_string(),
        name: format!("{} {}", vendor.to_uppercase(), device_code),
        utilization_percent: read_u64(device.join("gpu_busy_percent")).map(|value| value as f64),
        memory_total_bytes: read_u64(device.join("mem_info_vram_total")),
        memory_used_bytes: read_u64(device.join("mem_info_vram_used")),
        temperature_celsius,
        power_watts: first_hwmon_value(device, "power1_average")
...
fn first_hwmon_value(device: &Path, file_name: &str) -> Option<u64> {
