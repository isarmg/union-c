...
const blogAssetRoot = resolve(projectRoot, "data", "files");
const blogContentRoot = resolve(projectRoot, "data", "content");
...
export default defineConfig({
  ...
  vite: {
    ...
    server: {
      fs: {
        // 允许 Vite 开发服务器读取 blog 项目目录下的 source/ 和 data/。
        allow: [projectRoot]
      }
    }
  },
...
/**
 * 开发模式下把 blog/data/content 和 blog/data/files 加入 Vite 文件监听。
 * 这两个目录在 blog/source/ 外部，Vite 默认不监听，导致控制台写入新文章或
 * 修改现有文章后博客开发服务器不刷新内容。
 */
...
/**
 * 开发模式下把 /blog-assets/* 请求映射到 blog/data/files/ 目录。
