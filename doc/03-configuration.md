...
| 路径 | 用途 |
| --- | --- |
| `data/union.secret` | 仅开发模式自动生成的本地主密钥。 |
| `data/blog/content/` | PostgreSQL 导出的文章、站点配置和分类索引。 |
| `data/blog/files/` | 博客图片和附件源文件。 |
| `data/blog/logs/` | Astro 构建日志。 |
| `data/ram/files/` | ram 服务根目录。 |
| `data/ram/files/public/` | 公开文件。 |
| `data/ram/files/inbox/` | 上传暂存。 |
| `data/ram/files/private/` | 私有文件。 |
| `data/ram/files/media/` | 媒体文件。 |
| `data/ram/logs/` | ram 访问日志和进程日志。 |
| `data/ram/ram.generated.yaml` | union 生成的私有 ram 配置。 |
| `data/sunshine/` | Sunshine 相关运行数据与日志。 |
...
- 工作目录：`back/blog`；
- 构建命令：`npm run build`；
- 内容目录：`data/blog/content`；
- 资源目录：`data/blog/files`；
